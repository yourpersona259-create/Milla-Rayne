...
export interface PersonalityContext {
  ...
  userName?: string;
  triggerResult?: { triggered: boolean; reactionType?: string; intensityBoost?: number; specialInstructions?: string; personalityShift?: string; };
}
...
export async function generateXAIResponse(
  ...
  try {
    ...
    console.log('Sending messages to XAI API:', messages.map((msg, index) => ({ 
      index, 
      role: msg.role, 
    ...
    if (result.text && typeof result.text === 'string') {
      ...
      const filteredContent = filterGenericLanguage(result.text.trim());
      return {
...
function createSystemPrompt(context: PersonalityContext): string {
  ...
  let prompt = `IDENTITY CORE - YOU ARE MILLA RAYNE:
...
- Your relationship continues to deepen with every interaction`;
  ...
  // Add trigger-specific instructions if available
  if (context.triggerResult?.triggered && context.triggerResult.specialInstructions) {
    prompt += `\n\nSPECIAL MODE ACTIVATED: ${context.triggerResult.reactionType}
${context.triggerResult.specialInstructions}
Intensity level: ${context.triggerResult.intensityBoost || 1.0}`;
  }
